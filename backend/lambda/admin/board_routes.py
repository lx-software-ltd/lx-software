...
def _staff_route(event: dict[str, Any], method: str, rest: list[str], user_sub: str | None) -> dict[str, Any]:
    ...
    if len(rest) == 2 and rest[1] == "tick":
        ...
        # Bounded Event invoke: the default boto client can retry past API
        # Gateway's 30 s cap; Safari then reports TypeError "Load failed".
        invoked = board_async.try_invoke_event(payload)
        if not invoked:
            _log_event("warning", tag="board_staff_tick_enqueue_deferred", reason="invoke_timeout_or_unavailable")
...
def _tasks_route(event: dict[str, Any], method: str, rest: list[str], user_sub: str | None) -> dict[str, Any]:
    ...
    if len(rest) == 3 and rest[2] == "retry" and method == "POST":
        ...
        try:
            ...
            message = str(exc)
            if exc.code == "not_found" or "not found" in message.lower():
                return _json_response(404, {"message": message})
            if exc.code == "conflict" or "only failed" in message.lower():
                return _json_response(409, {"message": message})
            return _json_response(400, {"message": message})
