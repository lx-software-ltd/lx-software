...
| `BoardTaskDrawer` | `BoardOffcanvas` for one task: brief, step log (`<details>` + `BoardToolCallList`), deliverable (`BoardMarkdown`, CSV table, or a `messages` list with thread links), last review, failure reason, Accept / Return / Cancel (open rows) / Dismiss (failed) / Retry. Catalog micro-batch sheets also show a verified-fields import preview plus **Preview import** / **Import now** / **Skip import** / **Queue again** (owner `POST /catalog/preview` with `{remote:true}`, `/catalog/import`, `/catalog/skip`, `/catalog/requeue`). **Preview import** becomes **Previewing…** while the remote dry-run is in flight and the panel shows `local dry-run` / `remote dry-run`. Skip and requeue errors render in the same catalog message slot as preview/import. Import is disabled when the kill switch is off or the sheet was already imported. Accepted catalog sheets sit in **In progress** with a **To import** tag (`awaiting_import`); name collisions stay on **Attention** until the owner imports anyway, queues again, or skips (Accept / Return stay hidden on import-parked sheets). |
...
| `BoardSettingsCard` | Settings card: models, daily budget, **Enable staff tasks**, **Concurrent tasks** (`staff.maxRunningTasks`, 1–20), **Staff daily budget**, **Run scheduled seat duties** (`staff.dutiesEnabled`), digest To address. Duties stay off until the owner flips that switch even when staff is on. Model fields note that a 429 on the primary (DeepSeek shared pool) fails over to the stack defaults. |
...
| `BoardProgressSection` | **Progress** tab: KPI strip (listings, 7-day views, vendor signings, weekly partnerships, next-7-days content), bottleneck list with links into Pipeline / Content / Receivables, then district catalog (completeness plus photo / price / hours / geo), stalled onboarding, stalled outreach and stale drafts. Polls `GET /siu-tin-dei/board/progress` every 30s. |
| `BoardMarketSection` | **Market** tab: add/edit watchlist, candidates with Promote/Ignore, change-note timeline with before/after digests, latest weekly brief (opens `BoardTaskDrawer`). Empty pages show **not readable**. |
