...
| `BoardMailView` | Company mail: mailbox chips with unread badges, search + unread / **Archived** toggles, thread list, and a detail pane. The inbox hides archived bulk (DMARC `rua=`, SES setup). Opening an unread thread marks it read. **Board's view** swaps in the masked (`contact#N` / `phone#N`) bodies a persona sees. |
