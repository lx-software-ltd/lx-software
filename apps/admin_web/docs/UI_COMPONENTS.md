...
| `BoardTasksSection` | Executive Board **Tasks** tab (next to Staff): status-chip strip, **New task** offcanvas, search / assignee / Board·List / Show finished toolbar, then a four-lane board (`Attention` = needs owner + review, `In progress` = running + waiting, `Queued`, `Done` = failed + delivered + cancelled). Phones default to **List** (`AdminDataTable`) and pick one lane via a select. Cards show a short task id (`#` + first 8 chars, copy copies the full id), assignee display name, origin chip, step/budget meter, SLA, and a contextual failure / review line. Failed cards keep **Retry** (`POST /tasks/{id}/retry`) and **Dismiss** (`POST /tasks/{id}/cancel`); those errors render on the card. Deep link `?tab=board&section=tasks&task=<id>` (or `?task=` / `?section=tasks`) opens the drawer; parent / help ids in the drawer are links. The Tasks tab badge is `needs_owner`. "Open task" from Next actions writes the same query string. |
