...
| `BoardStaffSection` | Executive Board staff roster grouped by manager (active toggle, tier select, brief editor via `AdminEditorSection`) and a six-column task board (`Queued / Running / Review / Needs owner / Delivered / Failed`). Failed cards show `failureReason` and a **Retry** button (`POST /tasks/{id}/retry`). **Run staff tick now** calls `POST /staff/tick` (`200 {queued}`, background) and refetches staff/tasks after `STAFF_TICK_REFETCH_DELAY_MS`. **New task** assigns work to a persona or active seat. The Staff tab badge is `needs_owner`. Event-sourced cards show mail / Meta / review origin. |
