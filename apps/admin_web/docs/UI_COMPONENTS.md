...
| `BoardSettingsCard` | Settings card: models, daily budget, **Enable staff tasks**, **Run scheduled seat duties** (`staff.dutiesEnabled`), digest To address. Duties stay off until the owner flips that switch even when staff is on. Model fields note that a 429 on the primary (DeepSeek shared pool) fails over to the stack defaults. |
