...
| `BoardReviewSection` | Default Executive Board section when staff is enabled. Proposal §9 order: headline, holds due (reuses `BoardHoldsList`), escalations, assisted placeholder, sample with **This was wrong**, market placeholder, breakers with Reset, suggestions Accept/Later, promotion placeholder. Digest preview link. |
