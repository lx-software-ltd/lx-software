...
| `BoardTasksSection` | Executive Board **Tasks** tab (next to Staff): **New task** form plus a six-column board (`Queued / Running / Review / Needs owner / Delivered / Failed`). Failed cards show `failureReason`, **Retry** (`POST /tasks/{id}/retry`), and **Dismiss** (`POST /tasks/{id}/cancel`, dismisses a failed row). Cancel/retry errors render as alerts above the board. The Tasks tab badge is `needs_owner`. Event-sourced cards show mail / Meta / review origin. "Open task" from Next actions opens this tab. |
