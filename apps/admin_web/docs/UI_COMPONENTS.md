...
| `BoardTasksSection` | Executive Board **Tasks** tab (next to Staff): **New task** form plus a six-column board (`Queued / Running / Review / Needs owner / Delivered / Failed`). Failed cards show `failureReason`, **Retry** (`POST /tasks/{id}/retry`), and **Cancel** (`POST /tasks/{id}/cancel`, dismisses a failed row). The Tasks tab badge is `needs_owner`. Event-sourced cards show mail / Meta / review origin. "Open task" from Next actions opens this tab. |
| `BoardTaskDrawer` | `BoardOffcanvas` for one task: brief, step log (`<details>` + `BoardToolCallList`), deliverable (`BoardMarkdown`, CSV table, or a `messages` list with thread links), last review, failure reason, Accept / Return / Cancel / Retry. |
