...
| `BoardTaskDrawer` | `BoardOffcanvas` for one task: brief, step log (`<details>` + `BoardToolCallList`), deliverable (`BoardMarkdown`, CSV table, or a `messages` list with thread links), last review, failure reason, Accept / Return / Cancel (open rows) / Dismiss (failed) / Retry. |
