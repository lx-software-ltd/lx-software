...
| `SiutindeiBoardOutreachSendingDomain` → `OUTREACH_SENDING_DOMAIN` | `partners.siutindei.com` | WP6 |
| `SiutindeiBoardOutreachFromLocalPart` → `OUTREACH_FROM_LOCAL_PART` | `partnerships` | WP6 |
| `PublicApiBaseUrl` → `PUBLIC_API_BASE_URL` (stack-wide; leave blank to use this stack's HTTP API URL) | existing HTTP API URL | unsubscribe links (WP6), newsletter confirm (WP8) |
