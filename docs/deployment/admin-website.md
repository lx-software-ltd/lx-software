...
- **Catalog import (Option A):** `POST /siu-tin-dei/board/catalog/preview` and
  ...
  user (`AdminInitiateAuth` + `USER_PASSWORD_AUTH`). `catalog_import`
  ...
  sheet attaches `importPreview` only. This repo cannot push siutindei;
  land these product PRs before flipping the switch:
  ...
  3. **`dry_run`** — `POST /admin/imports` with `dry_run: true` (no
