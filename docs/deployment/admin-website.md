...
already exist in the account. CDK **imports those names** and grants
`AdminApiFn` read; it does not try to create them again. Put the real
values in Secrets Manager (`ap-southeast-1`). The older `lxsoftware-admin-*`
set stays in the stack (CDK-created, unused) for a future LX Software
board. OpenRouter stays `lxsoftware-admin-openrouter-api-secret-*` because
statement parsing also uses it.

| Secret name | Dummy shape | Replace with |
|-------------|-------------|--------------|
| `lxsoftware-admin-siutindei-board-github-token` | PAT string | Fine-grained GitHub PAT (plain string). Needed for write tools, security alerts, and a higher rate limit. Reads of the public `siutindei` repo work without it. |
...
2. After the stack is deployed, open
   `lxsoftware-admin-siutindei-board-github-token` in Secrets Manager and put the
