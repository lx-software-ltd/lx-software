...
- **Executive Board outreach (WP6):** Prospects, sequences and cold-outreach holds. Dual staff flags still required. Places (Enterprise SKUs) + FEHD/EDB open data. Sends from `partnerships@{OutreachSendingDomain}` with List-Unsubscribe; public `GET/POST /public/outreach/unsubscribe/{token}` has no JWT. SES config set → SNS → SQS `lxsoftware-admin-siutindei-outreach-events`. Daily 08:00 HKT `…-board-targets`. **Pipeline** section. `prospector` is active by default. Owner DNS/Places key/SES verify stay owner tasks.
- **Executive Board content (WP7):** Calendar + Pillow cards (`backend/lambda/admin/fonts/`, `brand/`). `content_publish` holds until `slotAt` (`publish:{channel}`). Sunday 18:00 HKT plan and Monday 09:00 HKT readout. `AdminApiFn` memory is 1536 MB; `requirements.txt` pins Pillow (Docker arm64 bundling). **Content** section. `content-marketer` and `growth-specialist` are active by default.
- **Executive Board newsletter (WP8):** Double opt-in `POST /public/newsletter/subscribe` plus confirm/unsubscribe tokens (no JWT). Issues send via SES `SendBulkEmail` after a `publish:newsletter` hold, from `news@{BoardMailDomain}`. Config set `lxsoftware-admin-siutindei-newsletter` shares the WP6 SQS bounce path and adds OPEN/CLICK. Public site `NewsletterForm` needs `VITE_PUBLIC_API_URL`.
- **Executive Board duties (WP9):** `accountant`, `data-analyst` and `security-analyst` are active by default. Seat `duties[]` (HKT cron) run from the staff tick when `settings.staff.dutiesEnabled` is on. Hourly cache refresh diffs CloudWatch ALARM names and new security/GitHub alert ids into architect/CTO and security-analyst tasks. Dunning creates an accountant task instead of a reminder Approval when staff is on. Stand-up `boundarySuggestions` land on the daily review.
- **Executive Board engineering (WP10):** Tool `code` (`board_code.py`) dispatches siutindei Actions workflows. `code_merge_staging` is a 12 h `code_staging` hold after CI + architect accept + path/size guards. `code_promote` is always an Approval; the review page **Promote** button queues it. Architect / engineer-1 / engineer-2 / product-dev are active by default. GitHub token needs `actions: write`. Workflows themselves live in the siutindei repo (appendix A).
