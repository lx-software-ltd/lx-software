...
- **OpenRouter bill:** LX Software pays one OpenRouter invoice. App tags live in `contracts/openrouter-apps.json` (this admin’s statement parser and Executive Board, plus Evolve Sprouts and the Siu Tin Dei product). Book UTC month-to-date from **LX Software → Dashboard → OpenRouter this month** (`GET /openrouter/usage`). Sibling repos tag requests with that catalog’s `referer` / `title` / `user {id}:{workload}` and an optional named key in the existing secret. See [`docs/deployment/admin-website.md`](docs/deployment/admin-website.md).
