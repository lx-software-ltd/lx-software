...
- **Executive Board receivables:** listing plans/subscriptions/invoices/payments live in siutindei Aurora. When `SiutindeiClusterArn` is set, this stack enables the RDS HTTP Data API and applies `scripts/siutindei/receivables.sql` (`SiutindeiDataApiSetup`; packaged copy in `backend/lambda/siutindei_schema/`). `AdminApiFn` uses the resolved secret (`SiutindeiDbSecretArn` or default name `lxsoftware-siutindei-database-credentials`). Product tools SELECT `v_catalog_health` / `v_funnel_daily` / `v_provider_pipeline` only. Issued invoices and matched payments are mirrored into the Siu Tin Dei statement book with stable `recv-inv-*` / `recv-pay-*` ids. The product CDK should set `enableDataApi: true` so a later siutindei deploy does not drift the HTTP endpoint off. Bank ingest is T4b (HK account not opened).
